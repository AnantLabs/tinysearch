...
/**
 * Base class for all filters. Provides common functionality applicable to all
 * filters.
 */
public abstract class Filter extends JComponent implements NodeFilter {
	...
	/**
	 * Map from cilter class to wrapper. Populated as part of each wrapper being
	 * loaded.
	 */
	protected static Hashtable mWrappers = new Hashtable();

	/**
	 * Create a filter. Set up the default display. Only a border with the label
	 * of the filter name, returned by <code>getDescription()</code>, and an
	 * icon, returned by <code>getIcon()</code>.
	 */
	public Filter() {
		JLabel label;
		Dimension dimension;
		...
		insets = getInsets();
		dimension.setSize(dimension.width + insets.left + insets.right,
	...
	/**
	 * Get the icon for the filter. Loads the resource specified by
	 ...
	 *                If the output stream complains (unlikely).
	 */
	 ...
	 *                If the serialized object class cannot be located.
	 */
	public static Object unpickle(byte[] data) throws IOException,
	...
	/**
	 * Serialize a byte array to a String. Convert each byte from the serialized
	 * object into a couple of hexadecimal characters.
	 * 
	 * @param data
	...
	public static byte[] deserialize(String string) {
		...
		for (int i = 0; i < string.length(); i += 2)
			ret[i / 2] = (byte) Integer
					.parseInt(string.substring(i, i + 2), 16); // todo:
																// hopelessly
																// inefficient
	 ...
	 * 
	 * @param string
	 *            The string with serialized node filters.
	 * @param context
	 *            The context from which to extract meaningful values for GUI
	 *            choices (which aren't serialized).
	...
	/**
	 * Get the enclosed sub filter list if any. Todo: rationalize with
	 * FilterBuilder's method(s) of the same name.
	 * 
	 * @param component
	 ...
	 * 
	 * @param filter
	 *            A filter to be wrapped by GUI components.
	 * @param context
	 *            The context within which to wrap the object. Some wrappers
	 *            need context to set up useful choices for the user.
		...
		if (null != class_name) {
			try {
				...
				if (0 != filters.length) {
					...
					if (null != list) {
						ret.setSubNodeFilters(new NodeFilter[0]); // clean out
																	// the
																	// unwrapped
																	// filters
						for (int i = 0; i < filters.length; i++)
						...
						throw new IllegalStateException(
								"filter can't have subnodes without a SubFilterList on the wrapper");
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	 ...
	 * 
	 * @param selected
	 *            If <code>true</code>, 'select' this component, otherwise
	 *            'deselect' it.
	 */
	public void setSelected(boolean selected) {
		if (selected)
			setBorder(new CompoundBorder(new EtchedBorder(),
					new CompoundBorder(new LineBorder(Color.blue, 2),
							new EmptyBorder(1, 1, 1, 1))));
		else
			setBorder(new CompoundBorder(new EtchedBorder(), new EmptyBorder(3,
					3, 3, 3)));
	}

	/**
	 * Set the expanded state for the component. This sets invisible all but the
	 * JLabel component in the comand component.
	 * 
	 * @param expanded
	 *            If <code>true</code>, 'expand' this component, otherwise
	 *            'collapse' it.
	 */
