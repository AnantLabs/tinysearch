...
/**
 * A layout manager that doesn't move things around. Attempts to set the bounds
 * of components to accomodate them but doesn't move them.
 */
		...
		synchronized (target.getTreeLock()) {
			...
			if (0 == count) {
				...
				if (null != parent) {
					...
					ret = parent.getSize();
					ret.setSize(ret.width - insets.left - insets.right,
				...
				for (int i = 0; i < count; i++) {
					...
					if (component.isVisible()) {
						...
						ret.width = Math.max(ret.width, point.x
								+ dimension.width);
						ret.height = Math.max(ret.height, point.y
								+ dimension.height);
					}
				}
