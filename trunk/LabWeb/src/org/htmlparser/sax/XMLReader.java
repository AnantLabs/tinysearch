 ...
 * 
 * <pre>
 * org.xml.sax.XMLReader reader = org.xml.sax.helpers.XMLReaderFactory
...
public class XMLReader implements org.xml.sax.XMLReader {
	...
	public void parse(InputSource input) throws IOException, SAXException {
		...
		if (null != mContentHandler)
			try {
				mParser = new Parser(new Lexer(new Page(input.getByteStream(),
						input.getEncoding())));
				...
				try {
					...
					for (NodeIterator iterator = mParser.elements(); iterator
							.hasMoreNodes(); doSAX(iterator.nextNode()))
						;
				...
				if (null != mErrorHandler)
					mErrorHandler.fatalError(new SAXParseException(pe
							.getMessage(), "", "", 0, 0));
			}
	}
		...
		if (null != mContentHandler)
			try {
				...
				if (null != mErrorHandler)
					...
					feedback = new DefaultParserFeedback(
	...
	protected void doSAX(Node node) throws ParserException, SAXException {
		...
		if (node instanceof Remark) {
			String text = mParser.getLexer().getPage().getText(
					node.getStartPosition(), node.getEndPosition());
			mContentHandler.ignorableWhitespace(text.toCharArray(), 0, text
					.length());
