 ...
 * must support this interface. More specific interface requirements for each of
 * the node types are specified by the {@link Text}, {@link Remark} and
 * {@link Tag} interfaces.
 */
public interface Node extends Cloneable {
	/**
	 * A string representation of the node. This is an important method, it
	 * allows a simple string transformation of a web page, regardless of a
	 * node. For a Text node this is obviously the textual contents itself. For
	 * a Remark node this is the remark contents (sic). For tags this is the
	 * text contents of it's children (if any). Because multiple nodes are
	 * combined when presenting a page in a browser, this will not reflect what
	 * a user would see. See HTML specification section 9.1 White space <a
	...
	/**
	 * Return the string representation of the node. The return value may not be
	 * the entire contents of the node, and non- printable characters may be
	 * translated in order to make them visible. This is typically to be used in
	 * the manner<br>
	 * 
	 * <pre>
	 ...
	 * 
	 * @return A string representation of this node suitable for printing, that
	 *         isn't too large.
	 */
	String toString();
	 ...
	 * {@link org.htmlparser.tags.CompositeTag}, and going through its
	 * children. So this method provides a convenient way to do this.
	 * </p>
	 * <p>
	 ...
	 * </pre>
	 * 
	 * Thus, <code>list</code> will hold all the link nodes, irrespective of
	 * how deep the links are embedded.
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param list
	 *            The list to collect nodes into.
	 * @param filter
	 *            The criteria to use when deciding if a node should be added to
	 *            the list.
	 *            </p>
	 */
	...
	/**
	 * Gets the starting position of the node. This is the character (not byte)
	 * offset of this node in the page.
	 * 
	...
	/**
	 * Gets the ending position of the node. This is the character (not byte)
	 * offset of the character following this node in the page.
	 * 
	...
	/**
	 * Get the parent of this node. This will always return null when parsing
	 * with the {@link org.htmlparser.lexer.Lexer}. Currently, the object
	 * returned from this method can be safely cast to a
	 ...
	 * 
	 * @return The next sibling to this node if one exists, <code>null</code>
	 *         otherwise.
	 */
	Node getNextSibling();
	 ...
	 * 
	 * @return The contents of the string or remark node, and in the case of a
	 *         tag, the contents of the tag less the enclosing angle brackets.
	 * @see #setText
	 */
	String getText();
	...
	/**
	 * Perform the meaning of this tag. This is defined by the tag, for example
	 * the bold tag &lt;B&gt; may switch bold text on and off. Only a few tags
	 * have semantic meaning to the parser. These have to do with the character
	 * set to use (&lt;META&gt;) and the base URL to use (&lt;BASE&gt;). Other
	 * than that, the semantic meaning is up to the application and it's custom
	 * nodes.<br>
	 ...
	 *                If a problem is encountered performing the semantic
	 *                action.
	 */
	...
	/**
	 * Allow cloning of nodes. Creates and returns a copy of this object. The
	 * precise meaning of "copy" may depend on the class of the object. The
	 * general intent is that, for any object <tt>x</tt>, the expression:
	 * <blockquote>
	 * 
	 * <pre>
	 * x.clone() != x
	 ...
	 * </pre>
	 * 
	 * </blockquote> will be <tt>true</tt>, this is not an absolute
	 * requirement.
	 * <p>
	 ...
	 * implement the interface <tt>Cloneable</tt>, then a
	 * <tt>CloneNotSupportedException</tt> is thrown. Note that all arrays are
	 * considered to implement the interface <tt>Cloneable</tt>. Otherwise,
	 ...
	 * are not themselves cloned. Thus, this method performs a "shallow copy" of
	 * this object, not a "deep copy" operation.
	 * <p>
	 * The class <tt>Object</tt> does not itself implement the interface
	 * <tt>Cloneable</tt>, so calling the <tt>clone</tt> method on an
	 * object whose class is <tt>Object</tt> will result in throwing an
	 ...
	 *                override the <code>clone</code> method can also throw
