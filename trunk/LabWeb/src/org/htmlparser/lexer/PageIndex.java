...
/**
 * A sorted array of integers, the positions of the first characters of each
 * line. To facilitate processing the first element should be maintained at
 * position 0. Facilities to add, remove, search and determine row and column
 * are provided. This class provides similar functionality to a Vector but does
 * not incur the overhead of an <code>Integer</code> object per element.
 */
public class PageIndex implements Serializable, Sortable {
	 ...
	 *            The page associated with this index.
	 * @param cursors
	 *            The initial elements of the index. NOTE: The list must be
	 ...
	 * @param cursor
	 *            The element to add.
	 * @return The position at which the element was inserted or the index of
	 *         the existing element if it is a duplicate.
	 */
	 ...
	 * 
	 * @return A new array containing the elements, i.e. a snapshot of the
	 *         index.
	 */
	public int[] get() {
	...
	/**
	 * Inserts an element into the list. The index must be a value greater than
	 * or equal to 0 and less than or equal to the current size of the array.
	 * 
	 * @param cursor
	...
	protected void insertElementAt(int cursor, int index) {
		if ((index >= capacity()) || (size() == capacity())) { // allocate more
																// space
			int[] new_values = new int[Math.max(capacity() + mIncrement,
					index + 1)];
			mIncrement *= 2;
			if (index < capacity()) {
				// copy and shift up in two pieces
				System.arraycopy(mIndices, 0, new_values, 0, index);
				System.arraycopy(mIndices, index, new_values, index + 1,
						capacity() - index);
			} else
			...
			// shift up
			System.arraycopy(mIndices, index, mIndices, index + 1, capacity()
					- (index + 1));
	...
	protected void removeElementAt(int index) {
		// shift
		System.arraycopy(mIndices, index + 1, mIndices, index, capacity()
				- (index + 1));
	 ...
	 *         would be (object.length - 1). For an empty index this will return
	 *         -1.
	 */
	public int last() {
		return (mCount - 1);
