 ...
 * @author derrick
 */
public class PicturePanel extends JPanel implements MouseListener, Scrollable,
	...
	/**
	 * The preferred size of this component. <code>null</code> initially,
	 * caches the results of <code>calculatePreferredSize ()</code>.
	 */
	 ...
	 * 
	 * @param thumbelina
	 *            The <code>Thumeblina</code> this panel is associated with.
	 */
	...
	/**
	 * Move the given picture to the top of the Z order. Adds it, even it if it
	 * doesn't exist. Also puts the URL in the url text of the status bar.
	 * 
	 * @param picture
	...
	/**
	 * Find a picture with the given URL in the panel. This should really only
	 * be used to discover if the picture is still visible. There could be more
	 * than one picture with the given URL because it may be partially obscured
	 * by another picture, in which case the pieces are each given their own
	 * picture object, but all point at the same <code>URL</code> and
	 * <code>Image</code>.
	 * 
	 * @param url
	 *            The url to locate.
	 * @return The first picture encountered in the panel, or null if the
	 ...
	 *            The picture to draw.
	 * @param add
	 *            If <code>true</code>, the picture is added to the history.
	 */
	...
	/**
	 * Paints this component. Runs through the list of tiles and for every one
	 * that intersects the clip region performs a <code>drawImage()</code>.
	 * 
	 * @param graphics
				...
				while (enumeration.hasMoreElements()) {
					...
					if ((null == clip) || (clip.intersects(picture))) {
						...
						if (!set.contains(image)) {
							...
							graphics
									.drawImage(picture.getImage(), origin.x,
											origin.y, origin.x + width,
											origin.y + height, 0, 0, width,
											height, this);
							set.add(image);
						}
					}
	...
	public Dimension getPreferredSize() {
		...
		else if ((0 == mPreferredSize.width) || (0 == mPreferredSize.height))
			setPreferredSize(calculatePreferredSize());
	 ...
	 * 
	 * @param dimension
	 *            The new value to use for <code>getPreferredSize()</code>
	 *            until recalculated.
	 */
	...
	/**
	 * Compute the preferred size of the component. Computes the minimum
	 * bounding rectangle covering all the pictures in the panel. It then does
	 * some funky stuff to handle embedding in the view port of a scroll pane,
	 * basically asking up the ancestor heirarchy what size is available, and
	 * filling it.
	 * 
	...
	/**
	 * Invoked when a mouse button has been released on a component. <i>Not
	 * used.</i>
	 * 
	 * @param event
	 *            The object providing details of the mouse event.
	...
	/**
	 * Invoked when the mouse enters a component. <i>Not used.</i>
	 * 
	 * @param event
	 *            The object providing details of the mouse event.
	 */
	...
	/**
	 * Invoked when the mouse exits a component. <i>Not used.</i>
	 * 
	 * @param event
	 *            The object providing details of the mouse event.
	 */
	 ...
	 * would effect the viewport size should just return getPreferredSize()
	 * here.
	 * 
	 ...
	 * <code>Scrollable</code> to match the width of the viewport. For example
	 * a normal text view that supported line wrapping would return true here,
	...
	/**
	 * Invoked when the component's position changes. <i>Not used.</I>
	 * 
	 * @param event
	 *            The component event.
	 */
	...
	/**
	 * Invoked when the component has been made visible. <i>Not used.</I>
	 * 
	 * @param event
	 *            The component event.
	 */
	...
	/**
	 * Invoked when the component has been made invisible. <i>Not used.</I>
	 * 
	 * @param event
	 *            The component event.
	 */
	...
	/**
	 * Handles this components ancestor being added to a container. Registers
	 * this component as a listener for size changes on the ancestor so that we
	 * may un-cache the prefereed size and force a recalculation.
	 * 
	 * @param event
	 *            The heirarchy event.
	 */
 ...
 * 
 * $Log: PicturePanel.java,v $ Revision 1.2 2005/04/12 11:27:41 derrickoswald
