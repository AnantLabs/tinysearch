 ...
 * {@link #Parser(URLConnection) URLConnection}, or a
 * {@link #Parser(Lexer) Lexer}. In the case of a String, a check is made to
 * see if the first non-whitespace character is a &lt;, in which case it is
 * assumed to be HTML. Otherwise an attempt is made to open it as a URL, and if
 * that fails it assumes it is a local disk file. If you want to parse a String
 * after using the {@link #Parser() no-args} constructor, use
 ...
 * {@.html <html>} node. This node is a {@link org.htmlparser.tags tag}, which
 * is an object of class {@link org.htmlparser.tags.Html Html} if the default
 * {@link NodeFactory} (a {@link PrototypicalNodeFactory}) is used.
 * </p>
 * <p>
 ...
 * </pre>
 * </code> would print out 5, not 2, because there are newlines after
 * {@.html <html>}, {@.html </head>} and {@.html </body>} that are children of
 * the HTML node besides the {@.html <head>} and {@.html <body>} nodes.
 * </p>
 * <p>
...
public class Parser implements Serializable, ConnectionMonitor {
	 ...
	 */
	public static final String VERSION_STRING = "" + VERSION_NUMBER + " ("
	...
	static {
		getConnectionManager().getDefaultRequestProperties().put("User-Agent",
				"HTMLParser/" + getVersionNumber());
	 ...
	 * @param resource
	 *            Either a URL, a filename or a string of HTML. The string is
	 *            considered HTML if the first non-whitespace character is a
	 ...
	 * @throws ParserException
	 *             If the resourceLocn argument does not resolve to a valid page
	 *             or file.
	 ...
	 *                supported, or an i/o exception occurs creating the lexer.
	 * @see #setLexer
	 * @see #getConnection
	 * @exception IllegalArgumentException
	 *                if <code>connection</code> is <code>null</code>.
	 * @exception ParserException
	 *                if a problem occurs in connecting.
	 */
	 ...
	 * prior to processing the page and <code>finishedParsing()</code> is
	 * called after the processing.
	 * 
	 * @param visitor
	 *            The visitor to visit all nodes with.
	 * @throws ParserException
	 *             If a parse error occurs while traversing the page with the
	 *             visitor.
	 */
	 ...
	 * @throws ParserException
	 *             If a error occurs in setting up the underlying Lexer.
	 * @exception IllegalArgumentException
	 *                if <code>inputHTML</code> is <code>null</code>.
	 */
