...
public class FrameSetTag extends CompositeTag {
	 ...
	 * <code>FRAMESET</code> tag suitable for debugging.
	 * 
	 * @return A string with this tag's contents.
	 */
	public String toString() {
		return "FRAMESET TAG : begins at : " + getStartPosition()
	...
	/**
	 * Gets a frame by name. Names are checked without case sensitivity and
	 * conversion to uppercase is performed with an English locale.
	 * 
	 * @param name
	...
	/**
	 * Gets a frame by name. Names are checked without case sensitivity and
	 * conversion to uppercase is performed with the locale provided.
	 * 
	 * @param name
	 *            The name of the frame to retrieve.
	 * @param locale
	...
	public FrameTag getFrame(String name, Locale locale) {
		...
		for (SimpleNodeIterator e = getFrames().elements(); e.hasMoreNodes()
				&& (null == ret);) {
			node = e.nextNode();
