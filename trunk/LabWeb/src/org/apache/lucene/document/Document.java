 ...
 * field may be {@link Fieldable#isStored() stored} with the document, in which
 * case it is returned with search hits on the document. Thus each document
 * should typically contain one or more stored fields which uniquely identify
 * it.
 * 
 * <p>
 * Note that fields which are <i>not</i> {@link Fieldable#isStored() stored}
 * are <i>not</i> available in documents retrieved from the index, e.g. with
 * {@link Hits#doc(int)}, {@link Searcher#doc(int)} or {@link
...
public final class Document implements java.io.Serializable {
	 ...
	 * always returns 1. This however does not mean that the boost value set at
	 * indexing time was ignored - it was just combined with other indexing time
	 * factors and stored elsewhere, for better indexing and search performance.
	 * (For more information see the "norm(t,d)" part of the scoring formula in
	 ...
	 * name. In this case, if the fields are indexed, their text is treated as
	 * though appended for the purposes of search.
	 * </p>
	 * <p>
	 * Note that add like the removeField(s) methods only makes sense prior to
	 * adding a document to an index. These methods cannot be used to change the
	 * content of an existing index! In order to achieve this, a document has to
	 * be deleted from an index and a new changed version of that document has
	 * to be added.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Removes field with the specified name from the document. If multiple
	 * fields exist with this name, this method removes the first field that has
	 * been added. If there is no field with the specified name, the document
	 * remains unchanged.
	 * </p>
	 * <p>
	 * Note that the removeField(s) methods like the add method only make sense
	 * prior to adding a document to an index. These methods cannot be used to
	 ...
	 * first value added. Do not use this method with lazy loaded fields.
	 */
	 ...
	 * <p>
	 * Note that fields which are <i>not</i>
	 * {@link Fieldable#isStored() stored} are <i>not</i> available in
	 * documents retrieved from the index, e.g. with {@link Hits#doc(int)},
	...
	/**
	 * Returns an array of {@link Field}s with the given name. This method can
	 * return <code>null</code>. Do not use with lazy loaded fields.
	 * 
	 * @param name
	...
	/**
	 * Returns an array of {@link Fieldable}s with the given name. This method
	 ...
	 * @param name
	 *            the name of the field.
	 * @return a <code>byte[]</code> containing the binary field value or
	 *         <code>null</code>
	 */
