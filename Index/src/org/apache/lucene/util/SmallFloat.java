...
public class SmallFloat {
	 ...
	 * @param numMantissaBits
	 *            the number of mantissa bits to use in the byte, with the
	 *            remainder to be used in the exponent
	 * @param zeroExp
	...
	public static byte floatToByte(float f, int numMantissaBits, int zeroExp) {
		...
		if (smallfloat < fzero) {
			return (bits <= 0) ? (byte) 0 // negative numbers and zero both
											// map to 0 byte
					: (byte) 1; // underflow is mapped to smallest non-zero
								// number.
	...
	/**
	 * floatToByte(b, mantissaBits=5, zeroExponent=2) <br>
	 * smallest nonzero value = 0.033203125 <br>
	 * largest value = 1984.0 <br>
	 * epsilon = 0.03125
	 */
