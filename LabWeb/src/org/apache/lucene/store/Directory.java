 ...
 * 
 * <p>
 * Java's i/o APIs not used directly, but rather all i/o is through this API.
 ...
 * </ul>
 * 
 * Directory locking is implemented by an instance of {@link LockFactory}, and
 * can be changed for each Directory instance using {@link #setLockFactory}.
 * 
 * @author Doug Cutting
 */
public abstract class Directory {
	...
	/**
	 * Holds the LockFactory instance (implements locking for this Directory
	 * instance).
	 */
	protected LockFactory lockFactory;

	/**
	 * Returns an array of strings, one for each file in the directory. This
	 * method may return null (for example for {@link FSDirectory} if the
	 * underlying directory doesn't exist in the filesystem or there are
	 ...
	 * the new name, then it is replaced. This replacement is not guaranteed to
	 * be atomic.
	 * 
	 * @deprecated
	 */
	...
	/**
	 * Returns a stream reading an existing file, with the specified read buffer
	 * size. The particular Directory implementation may ignore the buffer size.
	 * Currently the only Directory implementations that respect this parameter
	 * are {@link FSDirectory} and {@link
	 ...
	 * call this at a time when you are certain this lock is no longer in use.
	 * 
	 * @param name
	 ...
	 * locking implementation. Each * instance of LockFactory should only be
	 * used for one directory (ie, do not share a single instance across
	...
	/**
	 * Get the LockFactory that this Directory instance is using for its locking
	 * implementation. Note that this may be null for Directory implementations
	 * that provide their own locking implementation.
	 */
	 ...
	 * instance from other Directory instances. This ID should be the same if
	 * two Directory instances (even in different JVMs and/or on different
	 * machines) are considered "the same index". This is how locking "scopes"
	 * to the right index.
	 */
	 ...
	 * @param closeDirSrc
	 *            if <code>true</code>, call {@link #close()} method on
	 *            source directory
	 * @throws IOException
	 */
