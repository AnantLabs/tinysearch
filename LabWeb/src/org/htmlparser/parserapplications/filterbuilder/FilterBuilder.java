 ...
 * </ul>
 */
public class FilterBuilder extends JFrame implements WindowListener,
	...
	static {
		...
		dir = System.getProperty("user.home")
				+ System.getProperty("file.separator") + ".htmlparser";
		file = new File(dir);
		if (!file.exists())
	...
	/**
	 * Kludge: Used by actionPerformed/filterAction to remember the filter menu
	 * item.
	 */
	...
	public FilterBuilder() {
		...
		makeMenuButton("New", "Create a new document", "New", 'N', KeyStroke
				.getKeyStroke(KeyEvent.VK_N, Event.CTRL_MASK), toolbar, menu);
		makeMenuButton("Open", "Open an existing document", "Open...", 'O',
				KeyStroke.getKeyStroke(KeyEvent.VK_O, Event.CTRL_MASK),
				toolbar, menu);
		makeMenuButton("Save", "Save the active document", "Save...", 'S',
				KeyStroke.getKeyStroke(KeyEvent.VK_S, Event.CTRL_MASK),
				toolbar, menu);
		makeMenuButton("SaveAs", "Save the active document", "Save As...", 'A',
				KeyStroke.getKeyStroke(KeyEvent.VK_A, Event.CTRL_MASK), null,
				menu);
		menu.add(new JSeparator());
		makeMenuButton("Exit", "Exit the program", "Exit", 'E', KeyStroke
		...
		makeMenuButton("Cut", "Cut the selection and put it on the Clipboard",
				"Cut", 'T', KeyStroke.getKeyStroke(KeyEvent.VK_X,
						Event.CTRL_MASK), toolbar, menu);
		makeMenuButton("Copy",
				"Copy the selection and put it on the Clipboard", "Copy", 'C',
				KeyStroke.getKeyStroke(KeyEvent.VK_C, Event.CTRL_MASK),
				toolbar, menu);
		makeMenuButton("Paste", "Insert Clipboard contents", "Paste", 'P',
				KeyStroke.getKeyStroke(KeyEvent.VK_V, Event.CTRL_MASK),
				toolbar, menu);
		makeMenuButton("Delete", "Delete the selection", "Delete", 'D',
				KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, 0), toolbar, menu);
		menubar.add(menu);
		...
		addFilter(menu, toolbar,
				"org.htmlparser.parserapplications.filterbuilder.wrappers.AndFilterWrapper");
		addFilter(menu, toolbar,
				"org.htmlparser.parserapplications.filterbuilder.wrappers.OrFilterWrapper");
		addFilter(menu, toolbar,
				"org.htmlparser.parserapplications.filterbuilder.wrappers.NotFilterWrapper");
		menu.addSeparator();
		toolbar.add(new JToolBar.Separator());
		...
		addFilter(
				menu,
				toolbar,
				"org.htmlparser.parserapplications.filterbuilder.wrappers.HasAttributeFilterWrapper");
		menu.addSeparator();
		toolbar.add(new JToolBar.Separator());
		...
		addFilter(
				menu,
				toolbar,
				"org.htmlparser.parserapplications.filterbuilder.wrappers.HasParentFilterWrapper");
		addFilter(
				menu,
				toolbar,
				"org.htmlparser.parserapplications.filterbuilder.wrappers.HasChildFilterWrapper");
		addFilter(
				menu,
				toolbar,
				"org.htmlparser.parserapplications.filterbuilder.wrappers.HasSiblingFilterWrapper");
		menu.addSeparator();
		toolbar.add(new JToolBar.Separator());
		...
		item.addActionListener(this);
		menu.add(item);
		makeMenuButton("About",
				"Display program information, version number and copyright",
				"About", 'B', KeyStroke.getKeyStroke(KeyEvent.VK_H,
						Event.CTRL_MASK), toolbar, menu);
		menubar.add(menu);
		...
		mMainScroller = new JScrollPane(mMainPanel,
	...
	/**
	 * Creates a new instance of an FilterBuilder environment with the given
	 * title.
	 * 
	 * @param title
	 ...
	 */
	protected void makeMenuButton(String name, String description, String text,
			int mnemonic, KeyStroke key, JToolBar toolbar, JMenu menu) {
		JButton button;
		JMenuItem item;
		ImageIcon icon;
		String command;
	 ...
	 *                In the case that the document base or name of the resource
	 *                cannot be turned into a URL.
	 */
	protected URL getURL(String spec) throws MalformedURLException {
		URL ret;
	 ...
	 * @return A fully functional button with name, tool tip, icon and drag
	 *         recognizer.
	 */
	public JButton makeFilterButton(String class_name) {
		...
		if (null != filter) {
			ret.setName(class_name); // filter.getNodeFilter ().getClass
										// ().getName ());
			ret.setToolTipText(filter.getDescription());
			ret.setMargin(new Insets(0, 0, 0, 0));
			ret.setIcon(filter.getIcon());
			mDragSource.createDefaultDragGestureRecognizer(ret,
	...
	/**
	 * Add a filter to the GUI. Adds the filter specified by class_name to the
	 * menu, toolbar and starts listening for actions.
	 * 
	 * @param menu
	 *            The menu to add the filter to.
	 * @param toolbar
	 *            The toolbar to add the filter to.
	 * @param class_name
	 *            The class name for the filter wrapper. From the wrapper, the
	 *            NodeFilter, description and icon can be obtained.
	 */
	...
	/**
	 * Adds a set of filters to the main panel or a sublist. Sets up the GUI
	 * components as drop targets and mouse listeners, and performs a relayout
	 * to display them.
	 * 
	 * @param filters
	 *            The filter wrappers to add.
	 * @param point
	 *            The point at which to start adding (list == null).
	 * @param list
	 *            The list to add to (point not used), or <code>null</code>
	 *            for the main panel.
	 */
	protected void insertFilters(Filter[] filters, Point point,
			SubFilterList list) {
		Dimension dimension;
	 ...
	 * 
	 * @param point
	 *            The point where the mouse position is.
	 */
	...
	/**
	 * Gets the current base point of the mouse pointer. This value is used to
	 * offset the drag position to maintain the mouse position at the same
	 ...
	 * 		} else
	 * 			System.out
	 * 					.println(&quot;Usage: java -classpath .:htmlparser.jar Test &lt;url&gt;&quot;);
	 * 	}
	 * }
	 * </pre>
	 * 
	 ...
	 * @param bean
	 *            The bean to extract the filters from to make the program.
	 */
	protected void makeProgram(String name, StringBuffer out, FilterBean bean) {
		// so we need to keep track of filters and arrays of filters to give
		// them unique numbers
		// each Filter is responsible for outputting it's code and returning
		// it's variable name
		int[] context; // 0 - indent, 1 - next filter variable #, 2 - next
						// array of filters variable #
		String[] names;
		Filter[] filters;
		String array;
		...
		Filter.spaces(out, context[0]);
		out
				.append("System.out.println (\"Usage: java -classpath .:htmlparser.jar ");
		out.append(name);
		out.append(" <url>\");");
		Filter.newline(out);
	...
	/**
	 * Extracts a java class name from a file name. ToDo: make this
	 * package-smart somehow.
	 * 
	 * @param file
	...
	public void save(String name) {
		...
		if (0 != selections.length) {
			...
			// ToDo: grey out save option if nothing to save...
			JOptionPane.showOptionDialog(mMainPanel, "No filters to save.",
					"Oops", JOptionPane.DEFAULT_OPTION,
	...
	protected void pasteAction() {
		...
		if (content.isDataFlavorSupported(DataFlavor.stringFlavor)) {
			try {
				...
				filters = Filter.reconstitute(string, new Parser(mURLField
						.getText()));
	...
	protected void filterAction() {
		...
		try {
			filter = Filter.wrap(filter.getNodeFilter(), new Parser(mURLField
					.getText()));
	...
	protected void instructionsAction() {
		...
		// not .showMessageDialog(
				mMainPanel, instructions, "FilterBuilder Instructons",
				// remove this:
	...
	protected void filteringAction() {
		String instructions = "<html>"
				...
				+ "acceptable to a certain filter.</li>" + "</ul>" + "</html>";
		String close = "Close";
		JOptionPane.showOptionDialog(
		// not .showMessageDialog(
				mMainPanel, instructions, "FilterBuilder Instructons",
	...
	protected void tutorialAction() {
		String instructions = "<html>"
				...
				+ "<li>Compile the java file and run it.</li></ul>" + "</html>";
		String close = "Close";
		JOptionPane.showOptionDialog(
		// not .showMessageDialog(
				mMainPanel, instructions, "FilterBuilder Tutorial",
				// remove this:
				JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE,
				// and remove rest of these:
				null, new String[] { close }, close);
	...
	protected void hintsAction() {
		...
		JOptionPane.showOptionDialog(
		// not .showMessageDialog(
				mMainPanel, instructions, "FilterBuilder Hints",
				// remove this:
				JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE,
				// and remove rest of these:
				null, new String[] { close }, close);
	...
	/**
	 * Set up mouse listeners. Sets <code>this</code> up to listen to each
	 * command in the list as a MouseListener. Recursively descends the tree
	 * adding to all contained elements also.
	 * 
	 * @param filters
	 *            The container with commands in it.
	 */
	...
	/**
	 * Set up drop targets. Recursively descends the filter tree and sets up the
	 * filter lists as drop targets.
	 * 
	 * @param filters
	 *            The container with filters in it.
	 */
	 ...
	 * 
	 * @param filters
	 *            The list of filters to expand or collapse.
	 * @param expanded
	 *            If <code>true</code> the filters are expanded, otherwise
	 *            they are collapsed.
	 * @param recursive
	 *            If <code>true</code> the filters are processed recursively.
	 */
	public void setExpanded(Filter[] filters, boolean expanded,
			boolean recursive) {
		SubFilterList list;
		...
		try {
			...
			while (null != (line = reader.readLine()))
				if (line.startsWith("// [")) {
					...
					try {
						filters = Filter.reconstitute(line, new Parser(
								mURLField.getText()));
						mMainPanel.removeAll();
						point = new Point();
						for (int i = 0; i < filters.length; i++) {
	...
	/**
	 * Show a pop up context menu. Shows a context sensitive popup menu at the
	 * location of the mouse event.
	 * 
	 * @param event
	 *            The mouse event that initiates the popup.
	 */
	 ...
	 * 
	 * @param select
	 *            If <code>true</code> turn on highlighting, turn it off
	 *            otherwise.
	 */
	 ...
	 * 
	 * @param translation
	 *            The displacement to move them all by.
	 */
	 ...
	 * @param filter
	 *            The filter to check.
	 * @return <code>true</code> if the filter is a member, <code>false</code>
	 *         otherwise.
	 */
			...
			if (0 != (modifiers & InputEvent.SHIFT_MASK)) {
				...
				if (null == last)
					...
					if ((current != -1) && (recent != -1))
						for (int i = Math.min(current, recent); i <= Math.max(
								current, recent); i++)
	...
	/**
	 * Mouse drag notification. Invoked when a mouse button is pressed on a
	 * component and then dragged. Mouse drag events will continue to be
	 * delivered to the component where the first originated until the mouse
			...
			if (selectionContains(filter)) // drag on a selected item
			{
				if (null == getEnclosing(filter)) // not contained
					try {
						...
						if (null != base) {
							newpoint = event.getPoint();
							// translate the point relative to the enclosing
							// container
							upperleft = filter.getLocation();
							newpoint.translate(upperleft.x, upperleft.y);
							// get the difference between this point and the old
							// base
							translation = new Point(newpoint.x - base.x,
									newpoint.y - base.y);
	...
	/**
	 * Mouse move notification. Invoked when the mouse button has been moved on
	 * a component (with no buttons no down).
	 * 
	 * @param event
	...
	/**
	 * Invoked the first time a window is made visible. <i>Not used.</i>
	 * 
	 * @param event
	 *            The window event.
	 */
	...
	/**
	 * Handles window closing event. Performs function
	 ...
	 * the window's iconImage property. <i>Not used.</i>
	 * 
	 * @param event
	 *            The window event.
	 */
	 ...
	 * 
	 * @param event
	 *            The action even that triggers this function.
	 */
	 ...
	 * gesture. It is notifying this listener in order for it to initiate the
	 * action for the user.
	 * 
	 * @param event
	...
	public void dragGestureRecognized(DragGestureEvent event) {
		...
		try {
			cls = component.getName(); // (String)Filter.mWrappers.get
										// (component.getName ());
			if (null != cls) {
				filter = Filter.instantiate(cls);
				text = new StringSelection(Filter
						.deconstitute(new Filter[] { filter }));
				mDragSource.startDrag(event, DragSource.DefaultMoveDrop, text,
						this);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void drop(DropTargetDropEvent event) {
		...
		try {
			...
			if (transferable.isDataFlavorSupported(DataFlavor.stringFlavor)) {
				...
				s = (String) transferable
						.getTransferData(DataFlavor.stringFlavor);
				point = event.getLocation();
				try {
					// get the filter and add into the target
					filters = Filter.reconstitute(s, new Parser(mURLField
							.getText()));
					if (0 < filters.length)
	 ...
	 * 
	 * @param event
	 *            Details about the drop action change event.
	 */
	...
	/**
	 * The entry point for this application. Creates a new FilterBuilder and
	 * makes it visible.
	 * 
	 * @param args
