...
public class TagNameFilter implements NodeFilter {
	...
	/**
	 * Creates a new instance of TagNameFilter. With no name, this would always
	 * return <code>false</code> from {@link #accept}.
	 */
	...
	/**
	 * Accept nodes that are tags and have a matching tag name. This discards
	 * non-tag nodes and end tags. The end tags are available on the enclosing
	 * non-end tag.
	 * 
	 * @param node
	 *            The node to check.
	 * @return <code>true</code> if the tag name matches, <code>false</code>
	 *         otherwise.
	 */
	public boolean accept(Node node) {
		return ((node instanceof Tag) && !((Tag) node).isEndTag() && ((Tag) node)
				.getTagName().equals(mName));
	}
}
