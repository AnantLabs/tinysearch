...
/**
 * Handles proxies, password protected URLs and request properties including
 * cookies.
 */
public class ConnectionManager {
	/**
	 * Default Request header fields. So far this is just "User-Agent" and
	 * "Accept-Encoding".
	 */
	 ...
	 */
	private static final char[] BASE64_CHAR_TABLE = ("ABCDEFGHIJKLMNOPQRSTUVWXYZ"
	...
	/**
	 * Cookie storage, a hashtable (by site or host) of vectors of Cookies. This
	 * will be null if cookie processing is disabled (default).
	 */
	...
	/**
	 * Get the current default request header properties. A String-to-String map
	 * of header keys and values. These fields are set by the parser when
	 * creating a connection.
	 * 
	 * @return The default set of request header properties that will currently
	 *         be used.
	...
	/**
	 * Set the default request header properties. A String-to-String map of
	 * header keys and values. These fields are set by the parser when creating
	 * a connection. Some of these can be set directly on a
	 * <code>URLConnection</code>, i.e. If-Modified-Since is set with
	 * setIfModifiedSince(long), but since the parser transparently opens the
	 * connection on behalf of the developer, these properties are not available
	 * before the connection is fetched. Setting these request header fields
	 * affects all subsequent connections opened by the parser. For more direct
	 * control create a <code>URLConnection</code> massage it the way you want
	 * and then set it on the parser.
	 * <p>
	 * From <a href="http://www.ietf.org/rfc/rfc2616.txt"> RFC 2616 Hypertext
	 ...
	 * 
	 * @param properties
	 *            The new set of default request header properties to use. This
	 *            affects all subsequently created connections.
	 * @see #mDefaultRequestProperties
	 * @see #setRequestProperties
	 */
	...
	/**
	 * Get the current request header properties. A String-to-String map of
	 * header keys and values, excluding proxy items, cookies and URL
	 * authorization.
	 * 
	...
	/**
	 * Set the current request properties. Replaces the current set of fixed
	 * request properties with the given set. This does not replace the
	 * Proxy-Authorization property which is constructed from the values of
	 * {@link #setProxyUser} and {@link #setProxyPassword} values or the
	 * Authorization property which is constructed from the {@link #setUser} and
	 * {@link #setPassword} values. Nor does it replace the Cookie property
	 * which is constructed from the current cookie jar.
	 * 
	 * @param properties
	 ...
	 * 
	 * @return Returns the proxy user, or <code>null</code> if no proxy
	 ...
	 * 
	 * @return Returns the username that will be used to access the URL, or
	 *         <code>null</code> if no authorization is required.
	 */
	 ...
	 * 
	 * @param enable
	 *            if <code>true</code> cookie processing will occur, else
	 *            cookie processing will be turned off.
	 */
	...
	/**
	 * Enables or disables manual redirection handling. Normally the
	 * <code>HttpURLConnection</code> follows redirections (HTTP response code
	 * 3xx) automatically if the <code>followRedirects</code> property is
	 * <code>true</code>. With this flag set the
	 * <code>ConnectionMonitor</code> performs the redirection processing; The
	 * advantage being that cookies (if enabled) are passed in subsequent
	 * requests.
	 * 
	 * @param enabled
	...
	protected String getLocation(HttpURLConnection http) {
		...
		for (int i = 0; ((null == ret) && (null != (value = http
				.getHeaderField(i)))); i++)
			if ((null != (key = http.getHeaderFieldKey(i)))
			...
			try {
				try {
					...
					if ((null != getProxyHost()) && (0 != getProxyPort())) {
						...
						host = (String) sysprops.put("proxyHost",
								getProxyHost());
						port = (String) sysprops.put("proxyPort", Integer
								.toString(getProxyPort()));
						// see
						// http://java.sun.com/j2se/1.4.2/docs/guide/net/properties.html
						host2 = (String) sysprops.put("http.proxyHost",
								getProxyHost());
						port2 = (String) sysprops.put("http.proxyPort", Integer
								.toString(getProxyPort()));
					...
					if (ret instanceof HttpURLConnection) {
						...
						if (null != properties)
							for (enumeration = properties.keys(); enumeration
									.hasMoreElements();) {
						...
						if ((null != getProxyUser())
							...
							ret.setRequestProperty("Proxy-Authorization",
									encoded);
						}
						...
						if ((null != getUser()) && (null != getPassword())) {
							...
							encoded = encode(auth.getBytes("ISO-8859-1"));
							ret.setRequestProperty("Authorization", "Basic "
									+ encoded);
						}
	...
	/**
	 * Encodes a byte array into BASE64 in accordance with <a
	 ...
	 * provide URL encoding/decoding utilities).
	 * 
	 * @param url
	...
	public void addCookies(URLConnection connection) {
		...
		if (null != mCookieJar) {
			...
			if (null != host) { // http://www.objectsdevelopment.com/portal/modules/freecontent/content/javawebserver.html
				...
				if (null != domain)
					list = addCookies((Vector) mCookieJar.get(domain), path,
							list);
				else
	...
	protected String generateCookieProperty(Vector cookies) {
		...
		version = 0;
		for (int i = 0; i < cookies.size(); i++)
			version = Math.max(version, ((Cookie) cookies.elementAt(i))
					.getVersion());
		if (0 != version) {
