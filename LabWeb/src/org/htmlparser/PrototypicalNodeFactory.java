...
/**
 * A node factory based on the prototype pattern. This factory uses the
 * prototype pattern to generate new nodes. These are cloned as needed to form
 * new {@link Text}, {@link Remark} and {@link Tag} nodes.
 * <p>
 * Text and remark nodes are generated from prototypes accessed via the
 * {@link #setTextPrototype(Text) textPrototype} and
 ...
 * table. On a request for a tag, the attributes are examined for the name of
 * the tag to be created. If a prototype of that name has been registered
 * (exists in the hash table), it is cloned and the clone is given the
 * characteristics ({@link Attribute Attributes}, start and end position) of
 * the requested tag.
 * </p>
 * <p>
 * In the case that no tag has been registered under that name, a generic tag is
 * created from the prototype acessed via the
 * {@link #setTagPrototype(Tag) tagPrototype} property.
 * </p>
 * <p>
 * The hash table of registered tags can be automatically populated with all the
 * known tags from the {@link org.htmlparser.tags} package when the factory is
 * constructed, or it can start out empty and be populated explicitly.
 * </p>
 * <p>
 * Here is an example of how to override all text issued from
 * {@link org.htmlparser.nodes.TextNode#toPlainTextString()
 * Text.toPlainTextString()}, in this case decoding (converting character
 * references), which illustrates the use of setting the text prototype:
 * 
 * <pre>
 ...
 * </p>
 * <p>
 * Here is an example of using a custom link tag, in this case just printing the
 * URL, which illustrates registering a tag:
 * 
 * <pre>
...
public class PrototypicalNodeFactory implements Serializable, NodeFactory {
	...
	/**
	 * Create a new factory with all tags registered. Equivalent to
	 ...
	 * 
	 * @param empty
	 *            If <code>true</code>, creates an empty factory, otherwise
	 ...
	 * 
	 * @param id
	 *            The name under which to register the tag. <strong>For proper
	 *            operation, the id should be uppercase so it will be matched by
	 *            a Map lookup.</strong>
	 * @param tag
	 *            The tag to be returned from a {@link #createTagNode} call.
	 * @return The tag previously registered with that id if any, or
	 ...
	 * @return The tag registered under the <code>id</code> name, or
	 *         <code>null</code> if none.
	 */
	 ...
	 * @return The tag that was registered with that <code>id</code>, or
	 *         <code>null</code> if none.
	 */
	...
	/**
	 * Register a tag. Registers the given tag under every
	 * {@link Tag#getIds() id} that the tag has (i.e. all names returned by
	 * {@link Tag#getIds() tag.getIds()}.
	 * <p>
	 * <strong>For proper operation, the ids are converted to uppercase so they
	...
	/**
	 * Unregister a tag. Unregisters the given tag from every
	 * {@link Tag#getIds() id} the tag has.
	 * <p>
	 * <strong>The ids are converted to uppercase to undo the operation of
	 * registerTag.</strong>
	 * 
	 * @param tag
	...
	/**
	 * Register all known tags in the tag package. Registers tags from the
	 * {@link org.htmlparser.tags tag package} by calling
	 * {@link #registerTag(Tag) registerTag()}.
	 * 
	 ...
	 * 
	 * @param text
	 *            The prototype for {@link Text} nodes. If <code>null</code>
	 *            the prototype is set to the default ({@link TextNode}).
	 ...
	 * 
	 * @param remark
	 *            The prototype for {@link Remark} nodes. If <code>null</code>
	 *            the prototype is set to the default ({@link RemarkNode}).
	...
	/**
	 * Get the object that is cloned to generate tag nodes. Clones of this
	 * object are returned from {@link #createTagNode} when no specific tag is
	 * found in the list of registered tags.
	 * 
	...
	/**
	 * Set the object to be used to generate tag nodes. Clones of this object
	 * are returned from {@link #createTagNode} when no specific tag is found in
	 * the list of registered tags.
	 * 
	 * @param tag
	 *            The prototype for {@link Tag} nodes. If <code>null</code>
	 *            the prototype is set to the default ({@link TagNode}).
	 * @see #getTagPrototype
	 */
	...
	public Tag createTagNode(Page page, int start, int end, Vector attributes) {
		...
		if (null == ret) { // generate a generic node
			try {
