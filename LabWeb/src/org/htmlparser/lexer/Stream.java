...
public class Stream extends InputStream implements Runnable {
	...
	/**
	 * Construct a stream with no assumptions about the number of bytes
	 * available.
	 * 
	 * @param in
	 ...
	 *            The input stream to use.
	 * @param bytes
	 *            The maximum number of bytes to read. This should be set to the
	 *            ContentLength from the HTTP header. A negative or zero value
	 *            indicates an unknown number of bytes.
	 */
	...
	/**
	 * Fetch more bytes from the underlying stream. Has no effect if the
	 * underlying stream has been drained.
	 * 
	 * @param force
	 *            If <code>true</code>, an attempt is made to read from the
	 *            underlying stream, even if bytes are available, If
	 *            <code>false</code>, a read of the underlying stream will
	 *            not occur if there are already bytes available.
	...
	protected synchronized boolean fill(boolean force) throws IOException {
		...
		{
			if (!force) { // check for change of state while waiting on the
							// monitor in a synchronous call
				if (0 != available())
					return (true);
				synchronous++;
			}
			...
			if (0 == mContentLength) { // unknown content length... keep
										// doubling
				if (null == mBuffer) {
					...
					if (mBuffer.length - mLevel < BUFFER_SIZE / 2)
						buffer = new byte[Math.max(mBuffer.length * 2,
								mBuffer.length + mIn.available())];
					else
						buffer = mBuffer;
				}
				size = buffer.length - mLevel;
			} else { // known content length... allocate once
	...
	public void run() {
		...
		do { // keep hammering the socket with no delay, it's metered
				// upstream
			try {
				filled = fill(true);
	 ...
	 * 
	 * <ul>
	 * <li> If the method <code>mark</code> has not been called since the
	 ...
	 * 
	 * <ul>
	 * <li> The call to <code>reset</code> may throw an
	 ...
	 * 
	 * @exception IOException
	 *                <em>Never thrown. Just for subclassers.</em>
	 ...
	 * <code>markSupported</code> returns <code>true</code>, the stream
	 * somehow remembers all the bytes read after the call to <code>mark</code>
	 * and stands ready to supply those same bytes again if and whenever the
	 * method <code>reset</code> is called. However, the stream is not
	 * required to remember any data at all if more than <code>readlimit</code>
	 * bytes are read from the stream before <code>reset</code> is called.
	 * 
	 * @param readlimit
	 *            <em>Not used.</em>
	 * @see java.io.InputStream#reset()
