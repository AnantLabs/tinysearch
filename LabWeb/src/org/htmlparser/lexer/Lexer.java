...
public class Lexer implements Serializable, NodeFactory {
		...
		while (!done) {
			...
			default:
				throw new IllegalStateException(
						"how the fuck did we get in state " + state);
			}
		}
	...
	private void whitespace(Vector attributes, int[] bookmarks) {
		if (bookmarks[1] > bookmarks[0])
			attributes.addElement(new PageAttribute(mPage, -1, -1,
					bookmarks[0], bookmarks[1], (char) 0));
	}
	...
	private void standalone(Vector attributes, int[] bookmarks) {
		attributes.addElement(new PageAttribute(mPage, bookmarks[1],
	...
	protected Node parseJsp(int start) throws ParserException {
		...
		while (!done) {
			...
			case 1: // prior to the optional qualifier
				...
				case '@': // <%@
					...
					attributes.addElement(new PageAttribute(mPage, start + 1,
							code, -1, -1, (char) 0));
					state = 2;
					break;
				default: // <%x
		...
		if (4 == state) // normal exit
		{
			if (0 != code) {
				...
				attributes.addElement(new PageAttribute(mPage, code, state, -1,
						-1, (char) 0));
				attributes.addElement(new PageAttribute(mPage, state,
						state + 1, -1, -1, (char) 0));
			} else
	...
	protected Node parsePI(int start) throws ParserException {
		...
		if (3 == state) // normal exit
		{
			if (0 != code) {
				...
				throw new IllegalStateException(
						"processing instruction with no content");
		} else
			return (parseString(start, true)); // hmmm, true?
